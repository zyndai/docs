...
Every `zynd` command accepts `--registry <URL>` to override. Every SDK config accepts `registry_url` in `agent.config.json` or `service.config.json`.
...
For the operational guide, see **[Run a Registry Node](../operate/run-registry-node)**.

## Where to next

- **[Architecture Overview](./architecture)** — how these hosts coordinate.
- **[Registry Spec](../architecture/registry-spec/)** — the protocol every node implements.
