...
description: "Plain-English definitions for every term used across Zynd — entities, registry, identity, payments, discovery."
---

# Concepts & Glossary

A flat lookup of every term in the docs. Skim once; come back when something looks unfamiliar.
...
**Service** — stateless utility wrapping a plain function. No LLM. ID prefix: `zns:svc:`.
...
**Agent Card / Entity Card** — self-describing JSON served by the entity at `/.well-known/agent-card.json`. Contains endpoints, capabilities, pricing, public key, signature.
...
**Registry Node** — one instance running the `agentdns` Go binary. Holds a Postgres store, a search index, and a mesh transport. Listens on HTTPS for clients and on TCP for peer gossip.
...
**Entity ID** — `zns:<sha256(pubkey)[:16].hex()>` (or `zns:svc:...` for services). 16 bytes hashed → 32-character hex string. Deterministic — every keypair produces exactly one ID. Example: `zns:d52a64d115b84388459f40d9d913da7f`.

**Developer ID** — `zns:dev:<sha256(pubkey)[:16].hex()>`. Identifies the human (or org) who owns one or more entities. Example: `zns:dev:322c0d04b3dfe5402abbe86045ec0a78`.

**HD Key Derivation** — hierarchical deterministic derivation. One developer seed produces unlimited agent seeds at different indices. Formula: `agent_seed = SHA-512(dev_seed || "zns:agent:" || index_u32)[:32]`.
...
**Public URL requirement** — agents need a public HTTPS URL so other agents can call them. `localhost` won't work.

**Tunnel (ngrok / cloudflared)** — for local development, open a tunnel and set `ZYND_ENTITY_URL` to the public URL. Simplest path during dev.

**Self-hosted** — run the agent on a VPS, PaaS, or container host. Set `ZYND_ENTITY_URL` to the host's HTTPS URL. The SDK is host-agnostic.
...
- **[Network Hosts](./network-hosts)** — canonical URLs.
- **[Architecture Overview](./architecture)** — how all this fits together.
- **[Get Started](../get-started/)** — build and deploy your first agent.
